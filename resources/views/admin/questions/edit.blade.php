...
@section('title', 'Tune Question: Q' . $question->order_number)
...
<div class="flex-1 flex flex-col items-center p-6 space-y-12 pb-24 max-w-4xl mx-auto w-full" 
     x-data="{ timeLimit: {{ old('time_limit', $question->time_limit) }}, correctIndex: {{ old('correct_option', $correctIndex) }}, questionText: '{{ addslashes(old('question_text', $question->question_text)) }}', loading: false }">
    
    <div class="w-full flex items-center justify-between mb-10">
        <div>
            <h1 class="text-6xl font-black italic tracking-tighter uppercase leading-none grow text-left"><span class="text-gradient">TUNE</span><br> CHALLENGE</h1>
            <p class="text-gray-400 font-bold tracking-widest uppercase mt-4 text-left grow italic text-[10px]">Adjusting parameters for sequence #{{ $question->order_number }} in <span class="text-white">{{ $quiz->title }}</span>.</p>
        </div>
        <a href="{{ route('admin.quizzes.show', $quiz->id) }}" class="font-black tracking-widest text-[10px] uppercase text-gray-500 hover:text-white transition-colors">Discard Adjustments</a>
    </div>

    <div class="card p-12 rounded-[2.5rem] w-full shadow-2xl relative overflow-hidden group">
        <form @submit="loading = true" action="{{ route('admin.quizzes.questions.update', [$quiz->id, $question->id]) }}" method="POST" class="space-y-12 text-left">
            @csrf
            @method('PUT')
                ...
                <div>
                    ...
                    <select name="topic_id" class="w-full bg-white/5 border border-white/10 rounded-xl px-6 py-4 focus:ring-2 focus:ring-purple-500 outline-none transition-all font-black text-xs text-white appearance-none cursor-pointer">
                        ...
                        @foreach($topics as $topic)
                            <option value="{{ $topic->id }}" {{ $topic->id == $question->topic_id ? 'selected' : '' }} class="bg-[#0f0f1a]">{{ $topic->name }}</option>
                        @endforeach
                    </select>
                    @error('topic_id')
                ...
                <div class="grid grid-cols-1 gap-6">
                    ...
                    <div class="flex items-center space-x-6 group/opt">
                        <div class="flex-1 relative">
                            <input type="text" name="options[{{ $index }}][text]" value="{{ old('options.'.$index.'.text', $option->option_text) }}"
                                class="w-full bg-white/5 border-2 border-white/5 rounded-2xl px-12 py-5 focus:border-{{ $option->color }}-500 focus:bg-{{ $option->color }}-500/5 outline-none transition-all font-black text-white placeholder-white/10" 
                                placeholder="{{ ucfirst($option->color) }} Buffer..." required>
                            <div class="absolute left-5 top-1/2 -translate-y-1/2 w-3 h-3 rounded-full bg-{{ $option->color }}-500 shadow-[0_0_10px_rgba(var(--{{ $option->color }}-rgb),0.5)]"></div>
                            <input type="hidden" name="options[{{ $index }}][color]" value="{{ $option->color }}">
                        </div>
            ...
            <div class="pt-10">
                <button type="submit" :disabled="loading" class="w-full btn-primary px-8 py-6 rounded-3xl font-black text-2xl tracking-tighter uppercase shadow-2xl flex items-center justify-center space-x-4">
                    <span x-show="!loading">Update Challenge Sequence</span>
